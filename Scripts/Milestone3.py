...
from wordcloud import WordCloud  # To generate word cloud from text data
from textblob import TextBlob    # For basic NLP tasks like sentiment analysis
import json
...
# List of dataset file paths stored in Google Drive
dataset_paths = [
...
# Function to display the first few rows and basic info of each CSV file
...
""" ### Saving necessary files for Streamlit Dashboard"""

# Automatically adjust subplot params to give specified padding and prevent clipping
plt.tight_layout()

# Save the SHAP bar plot as a high-resolution PNG image
plt.savefig("shap_class1_bar.png", dpi=300, bbox_inches="tight")

# Print a confirmation message after saving the plot
print("Saved shap_class1_bar.png")

# Save the SHAP values DataFrame for class 1 to a CSV file
...
# cat > app.py << 'EOF'
# import os
# # 0) Force CPU-only for SHAP force_plot (avoids using GPU)
# os.environ["CUDA_VISIBLE_DEVICES"] = "-1"
# import matplotlib
# matplotlib.use("Agg")  # Use Agg backend for non-interactive plotting in Streamlit
# 
...
# st.set_page_config(
#     page_title="💊 MedRec Dashboard",  # Set the title for the page
#     layout="wide",  # Set the layout to wide for more space
#     page_icon="💊"  # Set a custom page icon
# )
# 
# # Custom header using markdown with a background color
# st.markdown("""
...
# 
# # 2) Load Lottie animation in the sidebar
# def load_lottie(url):
#     r = requests.get(url)  # Request the Lottie animation JSON from the URL
#     return r.json() if r.status_code == 200 else None  # Return the JSON if successful
# 
# with st.sidebar:
#     lottie_med = load_lottie("https://assets7.lottiefiles.com/packages/lf20_jbrw3hcz.json")  # Load the Lottie animation
#     if lottie_med:
#         st_lottie(lottie_med, height=200)  # Display the Lottie animation in the sidebar
# 
# # 3) Load model and data (using caching to avoid reloading every time)
# @st.cache_data(show_spinner=False)
# def load_artifacts():
#     model = joblib.load("best_rf_model.joblib")  # Load pre-trained Random Forest model
#     X_test = pd.read_csv("X_test.csv", index_col=0)  # Load test features (input data)
#     y_test = pd.read_csv("y_test.csv", index_col=0).values.ravel()  # Load test labels (actual values)
#     explainer = shap.TreeExplainer(model)  # Initialize SHAP TreeExplainer to explain model predictions
#     return model, X_test, y_test, explainer
# 
# with st.spinner("🚀 Loading model & data..."):  # Show a loading spinner while loading data
#     model, X_test, y_test, explainer = load_artifacts()  # Load artifacts using the caching function
#     y_probs = model.predict_proba(X_test)[:, 1]  # Get the predicted probabilities for class 1 (positive class)
# 
# # 4) Sidebar controls for user input
# st.sidebar.header("Controls")
# threshold = st.sidebar.slider("Classification threshold", 0.0, 1.0, 0.5, 0.01)  # Slider for classification threshold
# show_pr = st.sidebar.checkbox("Show PR Curve", True)  # Checkbox to show precision-recall curve
# show_cm = st.sidebar.checkbox("Show Confusion Matrix", True)  # Checkbox to show confusion matrix
# 
# # 5) Page navigation in the sidebar (radio button for page selection)
# page = st.sidebar.radio("Go to", [
#     "Metrics", "ROC Curve", "Feature Importances",
#     "SHAP: Bar", "SHAP: Beeswarm", "SHAP: Force"
# ])
# 
# # 6) Metrics page: Show model performance metrics
# if page == "Metrics":
#     st.header("Model Performance Metrics")
#     metrics = pd.read_csv("model_metrics.csv")  # Load performance metrics from a CSV file
#     st.dataframe(metrics, use_container_width=True)  # Display the metrics as a DataFrame
# 
# # 7) ROC Curve page: Display ROC curve image
# elif page == "ROC Curve":
#     st.header("ROC Curve Comparison")
#     st.image(
#         "roc_curve.png",  # Image of ROC curves for different models
#         caption="ROC Curves for Random Forest, SVM, and Logistic Regression",  # Image caption
#         use_container_width=False,  # Do not stretch the image to full width
#         width=600  # Set a fixed width for the image
#     )
# 
# # 8) Feature Importances page: Display bar chart for feature importances
# elif page == "Feature Importances":
#     st.header("Random Forest Feature Importances")
#     feat_imp = pd.read_csv("feature_importances.csv")  # Load feature importance values
#     fig, ax = plt.subplots(figsize=(8, 5))  # Create a figure for plotting
#     sns.barplot(x="Importance", y="Feature", data=feat_imp, ax=ax)  # Plot feature importances
#     ax.set_title("Feature Importances")  # Title for the plot
#     st.pyplot(fig, use_container_width=True)  # Display the plot in the Streamlit app
# 
# # 9) SHAP: Bar plot page: Show SHAP values as a bar chart for feature importance
# elif page == "SHAP: Bar":
#     st.header("SHAP: Mean |SHAP| per Feature")
# 
#     # 1) Sample 100 random instances from the test set
#     Xs = X_test.sample(100, random_state=42)
# 
#     # 2) Compute raw SHAP values
#     raw_vals = explainer.shap_values(Xs)
# 
#     # 3) Unify raw SHAP values into a 2D array (samples × features)
#     if isinstance(raw_vals, list):
#         shap_vals = raw_vals[1]  # Take SHAP values for the positive class (class 1)
#     elif isinstance(raw_vals, np.ndarray):
#         shap_vals = raw_vals
#         if shap_vals.ndim > 2:
#             shap_vals = shap_vals[..., 1]  # Pick class-1 SHAP values
#     else:
#         st.error(f"Unexpected SHAP output type: {type(raw_vals)}")
#         shap_vals = np.zeros((len(Xs), Xs.shape[1]))  # Default to a zero array if error occurs
# 
#     # 4) Compute mean absolute SHAP per feature
#     mean_abs = np.abs(shap_vals).mean(axis=0)
# 
#     # 5) Build a DataFrame for SHAP values
#     shap_df = pd.DataFrame({
#         "Feature": Xs.columns.tolist(),
#         "Mean |SHAP|": mean_abs.tolist()
#     }).sort_values("Mean |SHAP|", ascending=False)
# 
#     # 6) Display SHAP values DataFrame
#     st.dataframe(shap_df, use_container_width=True)
# 
#     # 7) Plot a horizontal bar chart for feature importance based on SHAP values
#     fig, ax = plt.subplots(figsize=(8, 5))
#     ax.barh(shap_df["Feature"], shap_df["Mean |SHAP|"])
#     ax.invert_yaxis()  # Invert y-axis so the most important feature is at the top
#     ax.set_xlabel("Mean |SHAP|")
#     ax.set_title("SHAP Feature Importances (Class 1)")
#     st.pyplot(fig, use_container_width=True)
# 
# # 10) SHAP Beeswarm page: Display global summary of SHAP values as a beeswarm plot
# elif page == "SHAP: Beeswarm":
#     st.header("SHAP Global Summary (Beeswarm)")
# 
#     # Display precomputed SHAP summary image
#     st.image(
#         "shap_summary_beeswarm.png",
#         caption="SHAP Global Summary (Beeswarm)",  # Caption for the image
#         use_container_width=False,  # Set a fixed width
#         width=500  # Adjust image size
#     )
# 
# # 11) SHAP Force page: Display SHAP Force plot for a single sample
# elif page == "SHAP: Force":
#     st.header("SHAP Force Plot (Sample 0)")
# 
#     # Create three columns for layout, centering the middle one
#     col1, col2, col3 = st.columns([1, 8, 1])
#     with col2:
#         st.image(
#             "shap_force_plot.png",  # Display precomputed SHAP force plot image
#             caption="SHAP Force Plot (pre-computed)",  # Caption for the plot
#             use_container_width=True  # Fill the column width with the image
#         )
# 
# # 12) Footer in sidebar with a message
# st.sidebar.markdown("---")
# st.sidebar.write("Built with ❤️ • Powered by Streamlit")  # Footer message
# EOF
#

# Commented out IPython magic to ensure Python compatibility.
# # Use 'sed' to replace deprecated Streamlit caching decorators with their updated versions in app.py
# %%bash
# sed -i \
